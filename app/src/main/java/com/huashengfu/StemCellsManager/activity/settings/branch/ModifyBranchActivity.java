...
public class ModifyBranchActivity extends BaseActivity {
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_settings_branch_modify);
        unbinder = ButterKnife.bind(this);

        app = (AppApplication) getApplication();
        rxPermissions = new RxPermissions(this);

        branch = (Branch) getIntent().getSerializableExtra(Constants.Tag.data);
        ...
        ivImage.setOnLongClickListener(new View.OnLongClickListener() {
            ...
            public boolean onLongClick(View view) {
                AlertDialog dialog = new AlertDialog.Builder(ModifyBranchActivity.this)
    ...
    private void init(){
        photos.add(branch.getPic());
        resetPhoto();

        etName.setText(branch.getName());
        tvCity.setText(branch.getAddr());
        etAddress.setText(branch.getAddr());
        tvTime.setText(branch.getBusinessHours());
        etPhone.setText(branch.getPhone());

        poiItem = new PoiItem("", new LatLonPoint(branch.getLatitude(), branch.getLongitude()), "", "");
//        08:30 - 17:30
        ...
        try{
            String[] tmp = branch.getBusinessHours().split(" - ");
            String[] begin = tmp[0].split(":");
            String[] end = tmp[1].split(":");

            calBegin.set(Calendar.HOUR_OF_DAY, Integer.parseInt(begin[0]));
            calBegin.set(Calendar.MINUTE, Integer.parseInt(begin[1]));
            ...
            case R.id.btn_commit:{
                ...
                if(files.isEmpty()){
                    try{
                        ...
                        obj.put(HttpHelper.Params.pic, oldPhotos.get(0));
                        obj.put(HttpHelper.Params.id, branch.getId());

                        String tmp = obj.toString().replaceAll("\\\\", "");
                        StringUtils.print(tmp);

                        OkGo.<JSONObject>put(HttpHelper.Url.Store.Branch.update)
                                .tag(this)
                    ...
                    OkGo.<JSONObject>post(HttpHelper.Url.Store.uploadBanner)
                            ...
                            .flatMap(new Function<Response<JSONObject>, ObservableSource<Response<JSONObject>>>() {
                                ...
                                public ObservableSource<Response<JSONObject>> apply(Response<JSONObject> jsonObjectResponse) throws Exception {
                                    ...
                                    Observable<Response<JSONObject>> observable = OkGo.<JSONObject>put(HttpHelper.Url.Store.Branch.update)
                                            .tag(this)
                                            .headers(HttpHelper.Params.Authorization, SPUtils.getInstance().getString(Constants.Tag.token))
                                            .upJson(tmp)
                                            .converter(new JsonConvert())
    ...
    private void goFinis(){
        showMessage(R.string.success_branch_modify);
        EventBus.getDefault().post(new RefreshBranchEvent());
        finish();
    }
    ...
    private class CameraPopupwindow{
        ...
        private void init() {
            ...
            popupWindow.setOnDismissListener(new PopupWindow.OnDismissListener() {
                ...
                public void onDismiss() {
                    ViewUtils.background(ModifyBranchActivity.this, 1f);
                }
            });
        ...
        public void show(){
            rxPermissions.requestEachCombined(new String[]{
                    ...
                    .subscribe(permission -> {
                        if(permission.granted){
                            ViewUtils.background(ModifyBranchActivity.this, 0.8f);
