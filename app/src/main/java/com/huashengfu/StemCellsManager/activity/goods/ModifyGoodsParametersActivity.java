...
// 编辑商品参数
public class ModifyGoodsParametersActivity extends BaseActivity {
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_goods_modify_parameters);
    ...
    private void doQuery(){
        OkGo.<JSONObject>get(HttpHelper.Url.Goods.Parameters.initByGoodsId + "?" + HttpHelper.Params.goodsId + "=" + goods.getGoodsId())
                .tag(this)
    ...
    public void onClick(View view){
        switch (view.getId()){
            ...
            case R.id.btn_next:{
                for(Parameters param : adapter.getParameters()){
                    if(StringUtils.isNullOrBlank(param.getDetails())){
                        showMessage("请输入" + param.getName());
                        return;
                    }
                }
                ...
                try {
                    ...
                    for(Parameters param : adapter.getParameters()){
                        ...
                        obj.put(HttpHelper.Params.name, param.getName());
                        obj.put(HttpHelper.Params.details, param.getDetails());
                        list.put(obj);
                    }

                    OkGo.<JSONObject>put(HttpHelper.Url.Goods.Parameters.modify + goods.getGoodsId())
                            .tag(this)
                            .headers(HttpHelper.Params.Authorization, SPUtils.getInstance().getString(Constants.Tag.token))
                            .upJson(list)
                            .execute(new DialogCallback<JSONObject>(this, false) {
                                ...
                                public void onSuccess(Response<JSONObject> response) {
                                    ...
                                    try {
                                        if(ResponseUtils.ok(response.body())){
                                            showMessage(R.string.success_goods_parameters_modify);
                                            finish();
                                        }else{
                                            showMessage(ResponseUtils.getMsg(response.body()));
                                        }
